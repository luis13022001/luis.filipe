...
void opcao(void)
{
    ...
    printf("7 - compacta fila \n");
    printf("8 - libera fila \n");
...
printf("Tamanho da lista que deseja criar (valor minimo  10)? ");
  scanf("%d", &tam);
    ...
    TQueue* alunos_ic = create_queue(10);
      for(int i=0; i< 10; i++){
        enqueue(alunos_ic, b[i]);
      }
break;  
...
break;
    case 3:
    ret = size_queue(alunos_ic);
    printf("o tamanho é : %d\n\n", ret);
  break;

    case 4: 
    ret = dequeue(alunos_ic);
    if( ret== -1 ){
      printf("ERRO!\n");
    }
break;
    ...
    case 5:
    if( queue_front(alunos_ic,&al) == SUCCESS ){
    printf("Matricula: %d\n",  al.matricula);
    printf("Nome: %s\n",   al.nome);
...
break;
    ...
    if(ret == -1){
        ...
        printf("Fila tem número negativo de elementos\n\n");
    }
    else{
        printf("A Fila não está vazia ela tem %d\n\n", ret);
    }
break;

    case 7:
    ret = compact_queue(alunos_ic);
    if(ret == -1){
        printf("ERROO!!!\n\n");
    }
    else{
        printf("Fila compactada com sucesso\n\n");
    }
break;

    case 8:
    ret = free_queue(alunos_ic);
    if(ret == -1){
        printf("ERROO!!!\n\n");
    }
    else{
